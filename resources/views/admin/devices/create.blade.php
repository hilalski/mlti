...
@section('title', 'Tambah Perangkat TI | Tim Jarkom')

@section('content')
<div class="pagetitle">
  <h1 class="fw-bold" style="color: var(--color-primary);"><i class="bi bi-laptop me-1"></i> Tambah Perangkat TI</h1>
  <nav>
    <ol class="breadcrumb">
    ...
    <div class="col-lg-8 col-md-10">
      ...
      <div class="card shadow-sm border-0">
        <div class="card-header d-flex justify-content-between align-items-center" style="background: linear-gradient(135deg, #FF84BA, #99C2FF) !important; padding: 15px 20px !important; margin: -24px -24px 20px -24px !important; border-top-left-radius: 12px; border-top-right-radius: 12px; border-bottom: none !important;">
          <h5 class="card-title my-0 text-white"><i class="bi bi-info-circle me-1"></i> Form Perangkat Baru</h5>
          ...
          <form action="{{ route('admin.devices.store') }}" method="POST">
            ...
            <div class="row g-3">
              ...
              <div class="col-md-6">
                ...
                <input type="text" name="id" id="id" value="{{ old('id') }}" class="form-control @error('id') is-invalid @enderror" required placeholder="Contoh: 3100102001-108">
                @error('id')
              ...
              <div class="col-md-6">
                ...
                <input type="text" name="brand" id="brand" value="{{ old('brand') }}" class="form-control @error('brand') is-invalid @enderror" required placeholder="Contoh: DELL, ASUS, Brother">
                @error('brand')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
              ...
              <div class="col-md-6">
                ...
                <input type="text" name="series" id="series" value="{{ old('series') }}" class="form-control @error('series') is-invalid @enderror" required placeholder="Contoh: Optiplex 3040, DCP-T710W">
                @error('series')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
              ...
              <div class="col-md-6">
                ...
                <input type="text" name="serial_number" id="serial_number" value="{{ old('serial_number') }}" class="form-control @error('serial_number') is-invalid @enderror" placeholder="Contoh: CN0Y13H6">
                @error('serial_number')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
              ...
              <div class="col-md-6">
                ...
                <input type="number" name="year" id="year" value="{{ old('year') }}" class="form-control @error('year') is-invalid @enderror" placeholder="Contoh: 2018">
                @error('year')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
              ...
              <div class="col-md-6">
                ...
                <select name="id_source" id="id_source" class="form-select @error('id_source') is-invalid @enderror" required>
                  ...
                  @foreach($sources as $source)
                    <option value="{{ $source->id }}" {{ old('id_source', 1) == $source->id ? 'selected' : '' }}>
                      {{ $source->asal }}
                    </option>
                  @endforeach
                </select>
              ...
              <div class="col-md-6">
                ...
                <select name="id_status_bmn" id="id_status_bmn" class="form-select @error('id_status_bmn') is-invalid @enderror" required>
                  ...
                  @foreach($statusBmns as $status)
                    <option value="{{ $status->id }}" {{ old('id_status_bmn', 1) == $status->id ? 'selected' : '' }}>
                      {{ $status->status }}
                    </option>
                  @endforeach
                </select>
              ...
              <div class="col-md-6">
                ...
                <select name="id_condition" id="id_condition" class="form-select @error('id_condition') is-invalid @enderror" required>
                  ...
                  @foreach($conditions as $cond)
                    <option value="{{ $cond->id }}" {{ old('id_condition', 1) == $cond->id ? 'selected' : '' }}>
                      {{ $cond->kondisi }}
                    </option>
                  @endforeach
                </select>
                ...
                <select name="id_user" id="id_user" class="form-select @error('id_user') is-invalid @enderror">
                  ...
                  <optgroup label="Pegawai / User">
                    @foreach($users as $user)
                      <option value="{{ $user->nip_lama }}" {{ old('id_user') == $user->nip_lama ? 'selected' : '' }}>
                  ...
                  <optgroup label="Ruangan">
                    @foreach($rooms as $room)
                      <option value="{{ $room->id }}" {{ old('id_user') == $room->id ? 'selected' : '' }}>
              ...
              <div class="col-md-12">
                ...
                <textarea name="keterangan" id="keterangan" rows="4" class="form-control @error('keterangan') is-invalid @enderror" placeholder="Tuliskan spesifikasi detail tambahan, riwayat upgrade, atau catatan lainnya tentang perangkat ini...">{{ old('keterangan') }}</textarea>
                @error('keterangan')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
            ...
            <div class="mt-4 pt-3 border-top text-end">
              ...
              <button type="submit" class="btn btn-primary btn-accent"><i class="bi bi-save"></i> Simpan Perangkat</button>
            </div>
